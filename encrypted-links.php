...
if (! function_exists('tfc_go_link')) {
    /**
     * Генерирует короткую ссылку (pretty или fallback) и продлевает transient.
     * Если отключены ЧПУ (permalink_structure пустой), используется формат ?tfc_go=HASH
     *
     * @param string $real_url Реальная ссылка назначения
    ...
    function tfc_go_link($real_url)
    {
        ...
        return add_query_arg('tfc_go', $hash, home_url('/'));
    }
}
...
{
    // Сначала регистрируем тег, затем правило (порядок на всякий случай)
...
// Добавим переменную в публичные query vars, чтобы работал fallback ?tfc_go=HASH даже без rewrite / flush.
function tfc_go_query_vars($vars)
{
    $vars[] = 'tfc_go';
    return $vars;
}
add_filter('query_vars', 'tfc_go_query_vars');

// Одноразовый flush при обновлении версии плагина.
define('TFC_GO_PLUGIN_VERSION', '1.1.1');
function tfc_go_maybe_flush()
{
    $stored = get_option('tfc_go_links_version');
    if ($stored !== TFC_GO_PLUGIN_VERSION) {
        tfc_go_register_rewrites();
        flush_rewrite_rules();
        update_option('tfc_go_links_version', TFC_GO_PLUGIN_VERSION, false);
    }
}
add_action('init', 'tfc_go_maybe_flush', 20);
...
function tfc_go_metabox_cb($post)
{
    ...
    $pretty = get_option('permalink_structure') ? '1' : '0';
    echo '<div id="tfc-go-links" data-home-url="' . esc_attr(home_url('/')) . '" data-pretty="' . esc_attr($pretty) . '">';
...
?>
    <script>
        (function() {
            ...
            const isPretty = container.getAttribute('data-pretty') === '1';
            ...
            function onInput(e) {
                ...
                shortInput.value = isPretty ? (home + '/go/' + hash + '/') : (home + '/?tfc_go=' + hash);
            }
            ...
            function onClick(e) {
                if (e.target.classList.contains('tfc-go-remove')) {
                    ...
                    if (rows.length > 1) {
                        e.target.closest('.tfc-go-row').remove();
                    }
                }
            }
            container.addEventListener('input', function(e) {
                if (e.target.classList.contains('tfc-go-real')) onInput(e);
            });
            ...
            function md5(e) {
                ...
                function d(e) {
                    return l(c(s(f(e), 8 * e.length)))
                }
                return d(unescape(encodeURIComponent(e)));
            }
        })();
    </script>
<?php
